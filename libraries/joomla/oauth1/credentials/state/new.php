 ...
 * @since       12.3
 */
class JOAuth1CredentialsStateNew extends JOAuth1CredentialsState
{
	/**
	...
	public function initialise($clientKey, $callbackUrl, $lifetime = 3600)
	{
		...
		$this->table->client_key = $clientKey;
		$this->table->key = $this->randomKey();
		$this->table->secret = $this->randomKey(true);
		$this->table->type = JOAuth1Credentials::TEMPORARY;
		$this->table->temporary_expiration_date = time() + $lifetime;

		// Persist the object in the database.
		$this->create();

		return new JOAuth1CredentialsStateTemporary($this->table);
	}
