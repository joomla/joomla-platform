 ...
 * @since       12.3
 */
abstract class JOAuth1ApplicationWeb extends JApplicationWeb
{
	/**
	 ...
	 *
	 * @param   object  $identity  An optional identity object. If omitted, the factory user is created.
	 *
	...
	private function _fetchCredentials($token, $consumerKey = null)
	{
		...
		$credentials = new JOAuth1Credentials;
		...
		if ($consumerKey == $credentials->getClientKey())
		{
			return $credentials;
		}
		else
	...
	private function _processPostVars()
	{
		...
		if (strtoupper($this->input->getMethod()) != 'POST'
			|| strtolower($this->input->server->get('CONTENT_TYPE', '')) != 'application/x-www-form-urlencoded')
		{
			return false;
		}
