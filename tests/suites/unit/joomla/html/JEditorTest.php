...
class JEditorTest extends PHPUnit_Framework_TestCase
{
	...
	public function testAttach()
	{
		...
		$this->editor->attach($observer);
		$this->assertEquals($this->editor->_methods, 
			array('__get' => array(1),
	...
	{
		// Verify it returns null if the 'none' plugin doesn't exist.
		$this->assertNull($this->editor->display('test', 'test', 'test', 'test', 'test', 'test', 'test', 'test',
		'test', 'test', 'test'));
	}
	...
	public function testDisplay($name, $html, $width, $height, $col, $row, $buttons, $id, $asset, $author, $params)
	{
		...
		$return = $this->editor->display($name, $html, $width, $height, $col, $row, $buttons, $id, 
		$asset, $author, $params);
		...
		$this->assertEquals($return, $plugin->onDisplay($name, $html, $width, $height, $col, $row, $buttons, $id));
	}
